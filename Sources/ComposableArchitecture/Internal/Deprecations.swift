...
// API.
  ...
  // `Store.withState(_:)` moved to `Store+WithState.swift`.
  ...
  // `TaskResultDebugging` moved to `TaskResultDebugging.swift` so it is
  // available on Android (where this file is otherwise gated on
  // canImport(SwiftUI)).
    ...
    extension UIAlertController {
      ...
      public convenience init<Action>(
        ...
        for button in state.buttons {
          self.addAction(
            .init(button, action: { store.send($0.map { .presented($0) } ?? .dismiss) }))
        }
