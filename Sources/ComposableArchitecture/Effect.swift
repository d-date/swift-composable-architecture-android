...
// On Android `MainActor`'s executor is not bound to a real thread (the
// process's main thread is the Android Looper, not libdispatch's main
// queue), so a `@MainActor`-isolated `Send` would force `Effect.run`
// bodies onto an executor that never drains. We therefore relax the
// isolation specifically on Android. Apple platforms keep the original
// upstream contract.
#if os(Android)
