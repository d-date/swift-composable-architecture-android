  ...
  // `Store.state`, `Store.subscript(dynamicMember:)`, and the
  // Equatable/Hashable/Identifiable conformances live in
  // `Store+ObservationCore.swift` so they remain available on platforms
  // without SwiftUI (Android / Linux).
  ...
  extension Optional: _ScopableState {
    public static func _scopeKeyPath<Root>(_ keyPath: KeyPath<Root, Self>) -> KeyPath<
      Root, Wrapped?
    > {
      keyPath
    }
